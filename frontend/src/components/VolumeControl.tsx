...
export const VolumeControl: React.FC<VolumeControlProps> = ({
  ...
  return (
    <div className={`flex items-center gap-4 px-4 group/vol w-full ${className}`.trim()}>
      <button
        type="button"
      ...
      >
        {isMuted ? <VolumeX size={iconSize} /> : <Volume1 size={iconSize} />}
      </button>
      ...
      <button
        ...
        className="text-[var(--text-muted)] hover:text-[var(--accent-primary)] shrink-0 transition-all active:scale-90"
        title="Set to 100%"
      >
